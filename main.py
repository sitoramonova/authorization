...
def init_db():
    ...
    with mysql.connector.connect(**db_config) as db:
        ...
        )  # Drop table if exists to create a new one
        cursor.execute(
            """
            CREATE TABLE tasks (
...
@bot.message_handler(
    func=lambda message: user_states.get(message.chat.id) == "waiting_for_description"
)
def get_task_description(message):
    ...
    bot.send_message(
        message.chat.id, "Введите дедлайн задачи (в формате ДД.ММ.ГГГГ ЧЧ:ММ):"
    )
...
def get_task_deadline(message):
    try:
        ...
        threading.Thread(
            target=reminder, args=(user_id, task_description, reminder_time)
        ).start()
        ...
        bot.send_message(
            message.chat.id, "Неверный формат даты. Пожалуйста, попробуйте снова."
        )
...
def reminder(user_id, task_description, reminder_time):
    ...
    bot.send_message(
        user_id,
        f"Напоминание: задача '{task_description}' должна быть выполнена завтра!",
    )
...
def show_tasks(message):
    ...
    with mysql.connector.connect(**db_config) as db:
        cursor = db.cursor()
        cursor.execute(
            "SELECT id, description, deadline FROM tasks WHERE user_id = %s AND completed = FALSE",
            (user_id,),
        )
...
def complete_task(message):
    bot.send_message(
        message.chat.id, "Введите ID задачи, которую хотите отметить как выполненную:"
    )
...
def mark_task_completed(message):
    ...
    if cursor.rowcount > 0:
        bot.send_message(
            message.chat.id, f"Задача с ID {task_id} отмечена как выполненная."
        )
    else:
